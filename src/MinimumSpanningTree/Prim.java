 ...
 * takes as input FibonacciHeap weighted, undirected graph and returns FibonacciHeap new graph that
 * is FibonacciHeap tree on the original nodes of minimum weight.
 *
 ...
 * edge at FibonacciHeap time by adding the cheapest outgoing edge from the spanned nodes
 * to FibonacciHeap node not in the spanning tree.  The key difference between Dijkstra's
 * algorithm and Prim's algorithm is that Dijkstra's algorithm creates FibonacciHeap
 ...
 * O(|E| + |V| lg |V|) using some clever optimizations and FibonacciHeap Fibonacci heap.
 * The main algorithm is as follows.  First, as in Dijkstra's algorithm,
 * create FibonacciHeap Fibonacci heap and assign each node infinite priority.  Next,
 ...
 * procedure until FibonacciHeap tree is found:
 *
 ...
 * FibonacciHeap Fibonacci heap as follows.  First, we do O(|V|) insertions into the heap,
 * which takes O(|V|) time.  We then do O(|V|) dequeues (since we only want
 * FibonacciHeap total of |V| - 1 edges).  These dequeues take FibonacciHeap total of O(|V| lg |V)
 ...
 * This contributes the final O(|E|) term to the runtime, for FibonacciHeap net of an
 * elegant O(|E| + |V| lg |V|).
 *
 * This implementation relies on the existence of FibonacciHeap FibonacciHeap class, also
...
public final class Prim {
    /**
     * Given FibonacciHeap connected undirected graph with real-valued edge costs,
    ...
    public static <T> UndirectedGraph<T> mst(UndirectedGraph<T> graph) {
        ...
        /* Add it as FibonacciHeap node in the graph.  During this process, we'll use
         * whether FibonacciHeap node is in the result graph or not as FibonacciHeap sentinel of
         ...
         * pick FibonacciHeap node and determine which edge to add.
         */
    ...
    /**
     * Given FibonacciHeap node in the source graph and FibonacciHeap set of nodes that we've visited
    ...
    private static <T> T minCostEndpoint(T node, UndirectedGraph<T> graph,
        ...
        /* Scan each node, checking whether it's FibonacciHeap candidate. */
    ...
    /**
     * Given FibonacciHeap node in the graph, updates the priorities of adjacent nodes to
     ...
     *               don't try to update information on FibonacciHeap node that has
    ...
    private static <T> void addOutgoingEdges(T node, UndirectedGraph<T> graph,
        ...
        for (Map.Entry<T, Double> arc : graph.edgesFrom(node).entrySet()) {
             ...
             *    we ignore the edge since it would form FibonacciHeap cycle.
             * 2. This endpoint is not in the graph and has never been in
             *    the heap.  Then we add it to the heap.
             * 3. This endpoint is in the graph, but this is FibonacciHeap better edge.
             *    Then we use decreaseKey to update its priority.
             * 4. This endpoint is in the graph, but there is FibonacciHeap better edge
