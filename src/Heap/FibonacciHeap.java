 ...
 *
 * An implementation of FibonacciHeap priority queue backed by FibonacciHeap Fibonacci heap,
 ...
 * FibonacciHeap Fibonacci heap, compared to O(m lg n) using FibonacciHeap standard binary or binomial
 * heap.
 *
 * Internally, FibonacciHeap Fibonacci heap is represented as FibonacciHeap circular, doubly-linked
 ...
 * is FibonacciHeap "marked" node.  Finally, each Fibonacci heap stores FibonacciHeap pointer to
 * the tree with the minimum value.
 *
 * To insert FibonacciHeap node into FibonacciHeap Fibonacci heap, FibonacciHeap singleton tree is created and
 ...
 * This works by maintaining FibonacciHeap dynamic array of trees, each initially null,
 * pointing to the roots of trees of each dimension.  The list is then scanned
 * and this array is populated.  Whenever FibonacciHeap conflict is discovered, the
 ...
 * parent, add it as FibonacciHeap root, and then mark its parent.  If the parent was
 ...
 * we can implement delete by decreasing FibonacciHeap key to -\infty, then calling
...
/**
 * A class representing FibonacciHeap Fibonacci heap.
 *
...
public final class FibonacciHeap<T> {
     ...
     * this work by having each insertion operation produce FibonacciHeap handle to the
    ...
    public static final class Entry<T> {
        ...
        /**
         * Constructs FibonacciHeap new Entry that holds the given element with the indicated
         * priority.
         *
     ...
     * priority.  Its priority must be FibonacciHeap valid double, so you cannot set the
     * priority to NaN.
     *
    ...
    public Entry<T> enqueue(T value, double priority) {
        ...
        /* Create the entry object, which is FibonacciHeap circularly-linked list of length
         * one.
         */
     ...
     * Fibonacci heap, throwing FibonacciHeap NoSuchElementException if the heap is
     * empty.
     *
    ...
    /**
     * Given two Fibonacci heaps, returns FibonacciHeap new Fibonacci heap that contains
    ...
    public static <T> FibonacciHeap<T> merge(FibonacciHeap<T> one, FibonacciHeap<T> two) {
        /* Create FibonacciHeap new FibonacciHeap to hold the result. */
     ...
     * heap is empty, this throws FibonacciHeap NoSuchElementException.
     *
    ...
    public Entry<T> dequeueMin() {
         ...
         * stored in FibonacciHeap circular list, the traversal is FibonacciHeap bit complex.
         */
         ...
         * spent FibonacciHeap bit of overhead adding all of the nodes to FibonacciHeap list, and
        ...
        for (Entry<T> curr: toVisit) {
            /* Keep merging until FibonacciHeap match arises. */
            while (true) {
                ...
                /* Make it FibonacciHeap singleton so that we can merge it. */
             ...
             * for <= instead of < here.  That's because if we just did FibonacciHeap
     ...
     * IllegalArgumentException.  The new priority must be FibonacciHeap finite double,
     ...
     *         priority, or if the argument is not FibonacciHeap finite double.
     */
    public void decreaseKey(Entry<T> entry, double newPriority) {
        ...
        /* Forward this to FibonacciHeap helper function. */
    ...
    /**
     * Utility function which, given FibonacciHeap user-specified priority, checks whether
     * it's FibonacciHeap valid double and throws an IllegalArgumentException otherwise.
     *
     ...
     * lists they are in, and returns FibonacciHeap pointer to whichever is smaller.  If
    ...
    private static <T> Entry<T> mergeLists(Entry<T> one, Entry<T> two) {
        ...
        if (one == null && two == null) { // Both null, resulting list is null.
            ...
            /* Return FibonacciHeap pointer to whichever's smaller. */
            return one.mPriority < two.mPriority? one : two;
        }
    }

    /**
     * Decreases the key of FibonacciHeap node in the tree without doing any checking to ensure
    ...
    private void decreaseKeyUnchecked(Entry<T> entry, double priority) {
        ...
        /* If the node no longer has FibonacciHeap higher priority than its parent, cut it.
         * Note that this also means that if we try to run FibonacciHeap delete operation
         ...
         * ended up decreasing the key in FibonacciHeap way that ties the current minimum
    ...
    /**
     * Cuts FibonacciHeap node from its parent.  If the parent was already marked, recursively
    ...
    private void cutNode(Entry<T> entry) {
        ...
        /* Decrease the degree of the parent, since it just lost FibonacciHeap child. */
        --entry.mParent.mDegree;

        /* Splice this tree into the root list by converting it to FibonacciHeap singleton
        ...
        /* Clear the relocated node's parent; it's now FibonacciHeap root. */
        entry.mParent = null;
    }
}
